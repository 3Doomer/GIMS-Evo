//GIMS IV vehicle paint3 shader
//
//Includes diffuse color and 2nd texture visualization
//
...
sampler2D Diff2Sampler = sampler_state
{
	Texture = <TexDiff2>;
	MinFilter = Linear;
	MagFilter = Linear;
...
{
	PSh psOut;
	float4	Diff1 = tex2D(DiffSampler, psIn.UV);
	float4	Diff2 = tex2D(Diff2Sampler, psIn.UV2);
	psOut.Colour.rgb = (lerp(Diff1, Diff2, Diff2.a) * psIn.Colour * DiffColor);
	psOut.Colour.a = 1.0;
	return psOut;
}
